...
public:
  static ClientRef create(std::string host = kp::opc::DEFAULT_IP,
                          int32_t port = kp::opc::DEFAULT_PORT,
                          bool isLongConnection = true,
                          bool isBandwidthConserved = true,
                          kp::opc::ColorOrder colorOrder = kp::opc::DEFAULT_COLOR_ORDER);
  ~Client();
