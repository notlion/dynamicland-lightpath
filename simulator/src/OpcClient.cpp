...
ClientRef Client::create(std::string host,
                         int32_t port,
                         bool isLongConnection,
                         bool isBandwidthConserved,
                         ColorOrder colorOrder) {
  return ClientRef(new Client(host, port, isLongConnection, isBandwidthConserved, colorOrder))
      ->shared_from_this();
}
...
void Client::setLED(int index, ci::Color8u color) {
  ...
  // Check for changes, only send a new message if needed (though mIsBandwidthConserved can
  // overrule)
